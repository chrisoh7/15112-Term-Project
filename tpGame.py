...
import time, tpMain, tpLeaderboard, random
...
def timerFired(app):
    if (app.isGameOver or app.winState) == True:
        return 0
    app.finalTime = time.time() - app.startTime
...
# for debugging purpose: winState testing
...
def drawPhysics(app, canvas):
    if not app.jumping:
        canvas.create_text(app.width / 2 - 42, unit * 11, text = 
                       f"""
                       v={app.vx}i + {0}j
                       g={app.a}
                       """, anchor = "c", fill = "black")
